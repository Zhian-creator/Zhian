...
Zhian is not invoked for every task. Repair tasks are first handled by the VulnRepair multi-agent pipeline driven by DeepSeek V4 Flash. Candidate patches undergo layered self-validation within the system, followed by a bounded feedback loop among root-cause analysis, patch generation, and revalidation based on failure attribution. Cases that still fail self-validation after the internal loop are escalated to Zhian. Zhian then performs an additional analysis and repair attempt using the crash evidence, root-cause analyses, candidate patches, and validation failures accumulated by the pipeline.
...
The repair backend is jointly driven by DeepSeek V4 Flash and the vulnerability-domain model Zhian (Zhian-27B). Candidate patches undergo layered self-validation within the VulnRepair multi-agent system, followed by a bounded feedback loop among root-cause analysis, patch generation, and revalidation based on failure attribution. Cases that still fail self-validation after the internal loop are escalated to Zhian, which performs an additional analysis and repair attempt using the accumulated failure evidence.
